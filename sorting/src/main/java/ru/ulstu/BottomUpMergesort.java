...
public class BottomUpMergesort<T extends Comparable<? super T>> {
     ...
     */
    private BottomUpMergesort(T[] array, int fromIndex, int toIndex) {
        if (array == null) throw new NullPointerException("The input array is null.");
    ...
    private void sort() {
        ...
        // 'runLength' by 2.
        for (int runLength = INSERTIONSORT_THRESHOLD; runLength < rangeLength; runLength <<= 1) {
    ...
    private void presortRuns(int runs) {
        ...
        for (int i = 0; i < runs - 1; ++i) insertionSort(source, localFromIndex, localFromIndex += INSERTIONSORT_THRESHOLD);
     ...
     */
    public static <T extends Comparable<? super T>> void insertionSort(T[] array, int fromIndex, int toIndex) {
        for (int i = fromIndex + 1; i < toIndex; ++i) {
            T element = array[i];
            int j = i;
            for (; j > fromIndex && array[j - 1].compareTo(element) > 0; --j) array[j] = array[j - 1];
            array[j] = element;
        }
    }
     ...
     * @param targetIndex the starting index of the result run in the target array.
     */
    ...
    private static void rangeCheck(int arrayLength, int fromIndex, int toIndex) {
        if (fromIndex > toIndex)
            throw new IllegalArgumentException("'fromIndex' is larger than 'toIndex': " + fromIndex + " > " + toIndex);

        if (fromIndex < 0)
            throw new ArrayIndexOutOfBoundsException("'fromIndex' is negative: " + fromIndex);

        if (toIndex > arrayLength)
            throw new ArrayIndexOutOfBoundsException("'toIndex' is too large: " + toIndex + ", array length: " + arrayLength);
    }
}
